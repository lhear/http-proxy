...
async def test_http_proxy(proxy: str, session: aiohttp.ClientSession, timeout: int, test_url: str) -> Tuple[str, bool, float, str, Optional[str]]:
    start_time = time.time()
    try:
        http_proxy_url = f"http://{proxy}"
        async with session.get(test_url, proxy=http_proxy_url, timeout=aiohttp.ClientTimeout(total=timeout), ssl=True) as response:
        ...
        response_time = time.time() - start_time
        return proxy, False, response_time, f"Unknown: {str(e)[:80]}", None

async def test_all_proxies(proxies: List[str], timeout: int, max_concurrent: int, test_url: str) -> List[Tuple[str, bool, float, str, Optional[str]]]:
    if not proxies:
        return []
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        async def worker(idx: int, proxy: str):
            async with semaphore:
                result = await test_http_proxy(proxy.strip(), session, timeout, test_url)
                results[idx] = result
                status = "✓" if result[1] else "✗"
                loc = f" loc={result[4]}" if result[4] else ""
                print(f"[{idx+1:5}/{len(proxies):5}] {status} {proxy:<25} | {result[2]:.2f}s | {result[3]}{loc}")
        tasks = [worker(i, p) for i, p in enumerate(proxies)]
...
def print_summary(results: List[Tuple[str, bool, float, str, Optional[str]]], round_num: int = None):
    ...
    header += f"\nTotal: {total_count}, Available: {available_count}, Success Rate: {available_count/total_count*100:.1f}%"
    header += f"\n{'='*80}"
    print(header)
...
def main():
    ...
    print(f"Reading proxy list from '{args.input_file}'...")
    ...
    if not current_proxies:
        print("Error: No proxies left after filtering")
        sys.exit(1)

    print(f"Starting test with {len(current_proxies)} proxies (concurrent={args.concurrent}, timeout={args.timeout}s)...")
    print(f"{'='*80}")
            ...
            if timestamp_file:
                for proxy, loc in available_proxies_with_loc:
                    if proxy not in proxy_info:
                        proxy_info[proxy] = {'added_at': current_run_start_time, 'location': loc or "N/A"}

            if len(available_proxies) == total:
                consecutive_success += 1
                print(f"Round {round_num} passed 100%! Consecutive: {consecutive_success}/3")
            else:
                consecutive_success = 0
                print(f"Round {round_num} failed 100%, resetting counter")
                current_proxies = available_proxies

            if not available_proxies:
                print("No proxies passed, terminating early")
                break

        if consecutive_success >= 3:
            print("\nPassed 3 consecutive rounds with 100% success!")
        else:
